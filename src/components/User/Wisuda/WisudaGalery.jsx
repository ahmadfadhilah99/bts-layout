...
const WisudaGalery = (props) => {
  ...
  const [wisudaIndex, setWisudaIndex] = useState(0);
  ...
  const getImageUrl = (path) => {
    ...
    return `${process.env.REACT_APP_API_KEY || ""}public/images/${path}`;
  };
    ...
    <div>
            ...
            <div className="flex justify-center">
              ...
              <div className="modal-content laptop:mx-3 -mx-1  ">
                ...
                {image[wisudaIndex].wisuda_gambar ? (
                  <img
                    src={getImageUrl(image[wisudaIndex].wisuda_gambar)}
                    alt={image[wisudaIndex].wisuda_nama || "Wisuda"}
                  />
                ) : (
              ...
              <div className="next mt-60">
                <button
                  className="bg-none md:bg-slate-600 sm:p-0 md:p-3 sm:w-auto md:w-14 sm:h-auto md:h-14 rounded-full"
                  disabled={wisudaIndex >= image.length - 1}
                  onClick={(e) => {
                    setWisudaIndex(wisudaIndex + 1);
                  }}
                >
      ...
      <div className="image-galery">
        {image.map((item, index) => (
          ...
          >
            <img
              src={getImageUrl(item.wisuda_gambar)}
              alt={item.wisuda_nama || "Wisuda"}
            />
          </div>
        ))}
      </div>
